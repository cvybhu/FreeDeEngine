...
vec3 fresnelSchlick(float cosTheta, vec3 F0) //F-resnel
{
    return F0 + (1.0 - F0) * pow(1.0 - cosTheta, 5.0);
}  


float DistributionGGX(vec3 N, vec3 H)  //normal D-istribution
{
...
float GeometrySmith(vec3 N, vec3 V, vec3 L)  //G-eometry
{
